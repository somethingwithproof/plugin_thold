			...
			if ($selected_items != false) {
				if (get_request_var('drp_action') == '1') { // delete
					db_execute_prepared('DELETE FROM plugin_notification_lists
						WHERE id IN (' . implode(',', array_fill(0, cacti_sizeof($selected_items), '?')) . ')',
						$selected_items);
					...
					db_execute_prepared('UPDATE host
						...
						AND deleted=""
						AND thold_host_email IN (' . implode(',', array_fill(0, cacti_sizeof($selected_items), '?')) . ')',
						$selected_items);
					...
					db_execute_prepared('UPDATE host
						...
						WHERE thold_host_email IN (' . implode(',', array_fill(0, cacti_sizeof($selected_items), '?')) . ')
						AND deleted=""',
						$selected_items);

					db_execute_prepared('UPDATE thold_data
						SET notify_warning = 0
						WHERE notify_warning IN (' . implode(',', array_fill(0, cacti_sizeof($selected_items), '?')) . ')',
						$selected_items);

					db_execute_prepared('UPDATE thold_data
						SET notify_alert = 0
						WHERE notify_alert IN (' . implode(',', array_fill(0, cacti_sizeof($selected_items), '?')) . ')',
						$selected_items);
					...
					for ($i = 0; ($i < count($selected_items)); $i++) {
						...
						if (get_request_var('notification_alert_action') > 0) {
							...
							if (get_request_var('notification_alert_action') == 1) {
								...
								db_execute_prepared('DELETE FROM plugin_thold_threshold_contact WHERE thold_id = ?', [$selected_items[$i]]);
							} else {
								// set the notification list
								db_execute_prepared('UPDATE thold_data
									SET notify_alert = ?
...
function tholds($header_label) {
	...
	if (!isempty_request_var('template') && get_request_var('template') != '-1') {
		$sql_where .= ($sql_where == '' ? '' : ' AND ') . 'td.data_template_id = ?';
	...
	if (strlen(get_request_var('rfilter'))) {
		$sql_where .= (!strlen($sql_where) ? '' : ' AND ') . 'td.name_cache RLIKE ?';
		$sql_params[] = get_request_var('rfilter');
	}
	...
	if (get_request_var('associated') == 'true') {
		$sql_where .= (!strlen($sql_where) ? '' : ' AND ') . '(td.notify_warning = ? OR td.notify_alert = ?)';
		$sql_params[] = get_request_var('id');
		$sql_params[] = get_request_var('id');
	}

	$result = get_allowed_thresholds($sql_where, $sort, $limit, $total_rows, 0, 0, $sql_params);
